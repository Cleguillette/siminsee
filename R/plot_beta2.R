#' We plot beta risk calculated over the repetitions for each scenario according to the three estimation approaches (EMR, EMR_INSEE and EMR_INSEE_IMP)
...
#'
#' @param mean_res : containg the data set containing estimations of coefficients over repetitions for each scenarios and the data set containing scenarios parameters from 'mean_coef_ana_2gr' function
#'
#' @return graphic of beta risk
#' @export


plot_beta2 <- function(mean_res){
  ...
  lambda_lfu1 <- lambda_lfu2 <- p_suc_uni <- NULL
  ...
  data_tmp <- data_tmp %>%
    ...
    pivot_longer(cols=c('p_suc_uni_true', 'p_suc_uni_emr', 'p_suc_uni_emr_insee', 'p_suc_uni_emr_insee_imp'), names_to = "p_suc_uni") %>%
    mutate(p_suc_uni = factor(p_suc_uni, levels=c('p_suc_uni_true', 'p_suc_uni_emr', 'p_suc_uni_emr_insee', 'p_suc_uni_emr_insee_imp')),
           value = 1 - value)


  beta_grp <- ggplot(aes(x = grp_insee, y = value, color =p_suc_uni), data=data_tmp) +
    geom_hline(yintercept = 0.2) +
    geom_point() +
    scale_colour_manual(values = c("#d175b8", "#FE6546", "#a5b800", "#45c1ff"),
                        labels=c("True data", 'EMR', 'EMR_INSEE', 'EMR_INSEE_IMP')) +
    labs(color = "Estimation approach", x= NULL, y = "Risk &beta;")  +
    facet_nested(rows = vars(lambda_d), cols = vars(hr_lfu, grp_lfu)) +
    theme_bw() +
    theme(strip.text.x = element_markdown(size=7), axis.title.y = element_markdown(),
